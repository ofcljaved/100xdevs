  ...
  return (
    <View className="flex-1" style={{ paddingBottom: insets.bottom }}>
        ...
        {state.routes.map((route, idx) => {
          ...
          return (
            <Button
              ...
              {drawerIcon ? drawerIcon({ size: 20, focused, color: '#545454' }) : null}
              {typeof label === 'string' ? (
                <Text className="text-base font-medium capitalize text-slate-600">{label}</Text>
              ) : (
      ...
      <View className="flex-1 justify-between border-t border-slate-200 p-4">
        <View>
          <Button variant={'link'} className="flex-row justify-start gap-6" onPress={handleLogout}>
